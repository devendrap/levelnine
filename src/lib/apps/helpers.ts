...
export async function getUIConfig(containerId: string, entityType: string): Promise<UIConfig | null> {
  const result = await query<UIConfig>(
    'SELECT * FROM container_ui_configs WHERE container_id = $1 AND entity_type = $2 AND is_active = true LIMIT 1',
...
export async function getAllUIConfigs(containerId: string): Promise<UIConfig[]> {
  const result = await query<UIConfig>(
    'SELECT * FROM container_ui_configs WHERE container_id = $1 AND is_active = true ORDER BY name',
    [containerId],
  )
  return result.rows
}
